...
2. **Leave the env, then move the session to the main checkout.** Which tool depends on how the session got into the env:
   ...
   - Otherwise (a session that was moved into the env with `mcp__ccd_directory__change_directory` instead of entering it, the opt-in flow), call `mcp__ccd_directory__change_directory` with the main checkout's path. It takes effect when the turn ends, so end the turn and continue from the next one.
