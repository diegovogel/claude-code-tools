...
**What it does:** puts a tiny tracked shim (`scripts/agent-env.sh`) in a project and keeps everything else outside the repo: the engine lives in the skill and the project's configuration in `~/.claude/agent-environments/<project>/project.sh`, so an engine fix reaches every project at once and a project's git history stays free of tooling churn. Each environment is a copy of the codebase and everything needed to run it. The skill is a stack-agnostic engine plus the guidance to write a small per-project config, so the same system adapts to Node, Laravel, WordPress, Python, and others. Day to day it's a handful of commands (which I integrate with other workflows): create an environment, serve it, list them, and destroy one when finished.
...
* Stack-agnostic by design. Only a small per-project config (a few scalars plus a few hooks) changes between stacks; the engine underneath is identical and exists in exactly one place, which is what lets it work in a stack it's never been tried in before.
