...
- Record facts about your own actions; don't re-derive them later from appearance. If code needs to know something that was certainly true at the moment it acted ("did I create this record?", "has this already been migrated?", "which run produced this file?"), write it down then. Detecting it afterwards from how the thing looks is guessing, and it fails whenever something else comes to look the same, or when a person edits the very properties you were reading. Repeated near-miss bugs in a detection rule are the signal: each fix closes one lookalike and the next one appears, because no appearance is unique to "I did this." Note the limit — this is about your own past actions. Questions about what something inherently *is* can legitimately be answered by inspecting it, provided the defining properties are actually definitional.
